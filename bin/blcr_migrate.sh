...
echo Migration command: cr_checkpoint -f $ckptfile --kill $cpid >> $ckpt_dir/ckpt.log 2>&1
echo $cpid > $ckpt_dir/pid.log
/usr/bin/cr_checkpoint -f $ckptfile --kill $cpid
cc=$?
if [ $cc -eq 0 ]; then
...
echo `date +"%D %T"` Job $1 "(pid=$2) checkpointed and killed, status=$cc" >> $ckpt_dir/ckpt.log
